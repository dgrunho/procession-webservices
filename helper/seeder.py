...
import model.gallery as gallery
...
# 39.603590
MIN_LONGITUDE = 39.59900
MAX_LONGITUDE = 39.610000

# - 8.415079
MIN_LATITUDE = - 8.415000
MAX_LATITUDE = - 8.40000


def parking_generate(parking_lot_number=10):
    parking_list = []

    for parking_lot in range(parking_lot_number):
        parking_list.append(
            parking.Parking(
                (random.uniform(MIN_LONGITUDE, MAX_LONGITUDE), random.uniform(MIN_LATITUDE, MAX_LATITUDE)),
                ('{} %'.format(int(random.uniform(0, 100))))
            ).__dict__
        )
    return parking_list


def gallery_generate(gallery_number=10):
    sample_url_size = len(SAMPLE_URLS) - 1

    if gallery_number > sample_url_size:
        gallery_number = sample_url_size

    gallery_list = []
    for i in range(gallery_number):
        gallery_list.append(SAMPLE_URLS[i])

    return gallery.Gallery(gallery_list).__dict__
...
def event_generate(event_number=10):
    event_list = []

    for e in range(event_number):
        time = datetime.now() + timedelta(hours=int(random.uniform(0, 10)))
        event_list.append(
            event.Event(
                'event {}'.format(e),
                format(time, '%Y-%m-%d %H:%M:%S'),
                '{} hours'.format(int(random.uniform(0, 8))),
                (random.uniform(MIN_LONGITUDE, MAX_LONGITUDE), random.uniform(MIN_LATITUDE, MAX_LATITUDE))
            ).__dict__
        )
...
def do_main():
    parking_list = parking_generate()
    for p in parking_list:
        print(p)

    gallery_list = gallery_generate()
    for g in gallery_list:
        print(g)

    events = event_generate(15)
